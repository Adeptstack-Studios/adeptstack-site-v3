...
type AppCardProps = {
    ...
    desc: string;
    icon: string;
    highlight?: boolean; // Optional, Standard ist false
};

export default function AppCard({ name, slogan, desc, icon, highlight = false }: AppCardProps) {
    return (
        <div
            className={`
      ...
      `}
        >
            ...
            <div className="absolute -right-20 -bottom-20 w-64 h-64 bg-blue-600/10 rounded-full blur-[80px] group-hover:bg-blue-600/20 transition duration-1000"></div>
            ...
            <div className="relative z-10 flex flex-col h-full justify-between">
                ...
                <div className="flex items-center gap-4 mt-8">
                    <button className="px-5 py-2.5 rounded-lg bg-white text-slate-950 font-bold text-sm hover:bg-blue-50 transition shadow-lg shadow-white/5">
                        Download
                    </button>
                    {highlight && (
                        <button className="px-5 py-2.5 rounded-lg text-white font-medium text-sm hover:bg-white/5 transition">
                            Features ansehen
                        </button>
                    )}
                </div>
